...
class CallService {
  ...
  init = () => {
    ...
    document.getElementById("createDialog").addEventListener("click", () => {
      ...
      ConnectyCube.chat.dialog
          .create(params)
          .then(dialog => {console.log('Dialog Object: ', dialog)})
          .catch(error => {});
    });
  };
  ...
  onRemoteConnectionStateChangedListener = (session, userId, iceState) => {
    console.warn('this.initiatorID', this.initiatorID);
    console.warn('this.participantIds', this.participantIds);
  ...
  startCall = () => {
    ...
    if (opponents.length > 0) {
      ...
      console.warn('this.janusRoomId', this.janusRoomId)
